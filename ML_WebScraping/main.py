...
class Scraper():
    ...
    def scraping(self):
        ...
        cleaned_name = self.product_name.replace(" ", "-").lower()
        ...
        for i in range(1, self.cantidad_paginas + 1):
            # Crear la URL para la página actual
            url = f"{self.base_url}{cleaned_name}_Desde_{(i-1) * 50 + 1}"
    ...
    def export_to_csv(self):
        ...
        # Generar el nombre del archivo CSV con el nombre del producto y la fecha y hora actual
        filename = f"data/{self.product_name}_{timestamp}.csv"

        # Exportar a un archivo CSV con nombres de columnas en español y codificación UTF-8
        df = pd.DataFrame(self.data, columns=["Producto", "Título", "Precio", "Enlace al Producto", "Enlace de la Imagen"])
